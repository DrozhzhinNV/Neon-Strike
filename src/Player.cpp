...
//  Player.cpp — управление и отрисовка игрока.


Player::Player()
    : health(C::PLAYER_MAX_HP), maxHealth(C::PLAYER_MAX_HP),
      speed(C::PLAYER_SPEED), currentWeaponIdx(0), resources(0)
{
    // --- Форма игрока: треугольник, нос вправо (0 градусов) ---
    body.setPointCount(3);
    body.setPoint(0, { 20.f,   0.f});  // нос (вперёд)
    body.setPoint(1, {-14.f, -13.f}); // хвост левый
    body.setPoint(2, {-14.f,  13.f}); // хвост правый
    body.setOrigin(3.f, 0.f);         // центр вращения примерно по центру
    body.setFillColor(sf::Color(50, 200, 255));
    body.setOutlineThickness(2.f);
    body.setOutlineColor(sf::Color(200, 240, 255));
    body.setPosition(C::MAP_W / 2.f, C::MAP_H / 2.f); // старт — центр карты

    // Маленький кружок на конце «ствола»
...
void Player::handleInput(float dt, const sf::RenderWindow& window) {
    ...
    // Нормализация: диагональ = та же скорость, что и по прямой
    ...
    // --- Уменьшить кулдаун текущего оружия ---
...
std::vector<Bullet> Player::tryShoot(const sf::RenderWindow& window) {
    ...
    // Стрелять только при зажатой ЛКМ и при готовности оружия
...
void Player::heal(float amount) {
    health = std::min(health + amount, maxHealth + maxHealthBonus);
}
...
void Player::drawHealthBar(sf::RenderWindow& window) const {
    ...
    float total = maxHealth + maxHealthBonus;
